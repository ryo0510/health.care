class History < ApplicationRecord
  ...
  def self.created_2day_ago #2日前
    time = find_by(start_time: 2.day.ago.midnight..2.day.ago.end_of_day)  #2日前
    time.nil? ? 0 : time.result
  end

  def self.created_3day_ago
    time = find_by(start_time: 3.day.ago.midnight..3.day.ago.end_of_day)  #3日前
    time.nil? ? 0 : time.result
  end

  def self.created_4day_ago
    time = find_by(start_time: 4.day.ago.midnight..4.day.ago.end_of_day)  #4日前
    time.nil? ? 0 : time.result
  end

  def self.created_5day_ago
    time = find_by(start_time: 5.day.ago.midnight..5.day.ago.end_of_day)  #5日前
    time.nil? ? 0 : time.result
  end
